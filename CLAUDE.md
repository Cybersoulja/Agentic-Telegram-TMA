...
**Storage layer** (`db.ts`): D1 is the source of truth; KV is a 3600s read-through/write-through cache keyed `user:{userId}`. `getUserProfile` checks KV first, falls back to D1, and repopulates KV on a D1 hit. All D1/KV errors are caught and logged, never thrown, so profile/activity failures degrade gracefully instead of breaking a request.

**Integrations hub** (`integrations.ts`): proxies/mocks three external local services — n8n workflow hub, a Qwen3-TTS voice studio, and a "NovelAI Lorebook Agent" sandbox. `GET .../status` health-checks n8n and Qwen over HTTP with a 2s timeout; `POST .../trigger` dispatches to one of `novelai` (mocked response), `n8n` (proxies to `${N8N_URL}/webhook/{action}`), or `qwen_tts` (mocked). These are local/dev-only integrations, not production services — treat their responses as illustrative stubs when extending.
