...
- `GET/POST /api/profile` — read/write a user profile, backed by D1 with a KV cache; gated by `authenticateInitData` (see below) — the profile id always comes from the verified `initData`, never from a client-supplied `userId`/`id`, so one user can't read or overwrite another's profile
- `POST /api/mission-log` — chains three integrations together: reads the latest Tier 5 Oracle narrative from `activity_logs`, mocks a Qwen3-TTS `audio_url` and a Bluesky post `uri` for it, and persists the result to the `mission_logs` table; gated by `authenticateInitData` too — the narrative itself isn't per-user data, but the gate stops this public Worker from being spammed into writing bogus rows / mock TTS+Bluesky calls
...
**initData verification** (`verifyTelegramInitData` in `telegramAuth.ts`): reimplements Telegram's documented HMAC-SHA256 check using only the runtime Web Crypto API (`crypto.subtle`) — no external crypto dependency. Also rejects data older than `maxAgeSeconds` (default 24h). Do not replace this with a third-party library. It's a standalone module (not defined in `index.ts`) specifically so `integrations.ts` and `rpg.ts` can import it too without creating a circular dependency on `index.ts`.

The same file also exports `authenticateInitData(initData, env, corsHeaders)` — a shared route-gate wrapper around `verifyTelegramInitData` that returns either `{ok: true, userId}` or `{ok: false, response}` (a ready-to-return 403/500 `Response`), so a call site just does `if (!auth.ok) return auth.response;`. Every route that touches a real per-user Cloudflare resource, a billed Gemini call, or a real Craft write uses this same helper: `/api/profile` and `/api/mission-log` in `index.ts`, every `/api/rpg/*` route in `rpg.ts`, and the `craft` action in `integrations.ts`. Add the same gate to any new route with the same property, rather than reinventing the check inline.
